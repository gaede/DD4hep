...
static Ref_t create_detector(LCDD& lcdd, xml_h e, SensitiveDetector sens) {
		...
		for (xml_coll_t li(x_det, _U(layer)); li; ++li) {
			...
			// Loop over number of repeats for this layer.
				for (int j=0; j<repeat; j++) {
					...
					DetElement layer(stave_det, l_name, l_num);
					...
					for(xml_coll_t si(x_layer,_U(slice)); si; ++si) {
						...
						DetElement slice(layer,s_name,s_num);
	...
	for (int i = 0; i < nsides; i++, phi -= dphi) { // i is module number
		...
		PlacedVolume pv = envelope.placeVolume(mod_vol, Position(-m_pos_x, -m_pos_y, 0), Rotation(phi, M_PI * 0.5, 0));
